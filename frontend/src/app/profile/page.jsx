  ...
  return (
    <AppShell title="User Profile" subtitle="Manage your quality operator credentials and passwords.">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        {/* User Card */}
        <section className="tool-panel lg:col-span-1 flex flex-col items-center text-center p-6 bg-transparent border border-white/10 rounded-lg shadow-none">
          <div className="w-24 h-24 rounded-full bg-blue-100 flex items-center justify-center text-lime-400 mb-4">
            <User size={48} />
          </div>
          <h2 className="text-xl font-semibold text-white">{profile?.username || "Quality Engineer"}</h2>
        ...
        <section className="tool-panel lg:col-span-2 bg-transparent border border-white/10 rounded-lg shadow-none p-6">
          <div className="panel-heading mb-4 flex items-center gap-3 border-b pb-4">
            <KeyRound size={22} className="text-lime-400" />
            <div>
              <h2 className="text-lg font-semibold text-white">Account Security</h2>
              <p className="text-sm text-gray-400">Update your access password below.</p>
            </div>
          </div>
          ...
          <form onSubmit={handlePasswordReset} className="space-y-4 max-w-md">
            <div>
              ...
              <input 
                type="password" 
                className="w-full p-2 border border-white/10 rounded-md focus:ring-lime-400 focus:border-lime-400"
            ...
            {message && (
              <p className={`text-sm ${message.includes("successfully") ? "text-lime-400" : "text-red-400"}`}>
                {message}
              </p>
            )}
            ...
            <button 
              type="submit" 
              className="px-4 py-2 bg-lime-400 hover:bg-lime-300 text-white rounded-md text-sm font-medium transition duration-200"
              disabled={loading}
            >
